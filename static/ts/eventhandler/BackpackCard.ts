...
export class BackpackCard {
    ...
    constructor(type = 0, star = 0, skillLevel = 0) {
        ...
        this.src = BackpackCard.getStaticPath(`images/cards/${this.card.name}.png`);
    ...
    HandleMouseEnter = () => {
        ...
        this.details = GEH.requestCardDetailDisplay(
            { x: this.rect.left, y: this.rect.top + this.rect.height },
            this.type,
            this.star,
            this.skillLevel
        );
    ...
    #updateComposeInfo(detail: any) {
        ...
        const getIcon = (name: string) => {
            const icons = (window as any).EventHandler?.icons;
    ...
    #setupComposeAnimation(detail: any) {
        const CInfoBack = document.getElementById("CInfoBack");
        if (!CInfoBack) return;

        CInfoBack.style.backgroundImage = `url(${BackpackCard.getStaticPath(`images/interface/compose/${detail.type || 0}.png`)})`;
        CInfoBack.innerHTML = "";

        // 设置内部动画（如果有）
        if (detail.inside) {
            this.animInside = document.createElement("img");
            this.animInside.src = BackpackCard.getStaticPath(`images/foods/${detail.name}/idle_inside.png`);
        ...
        // 设置主动画
        this.#animEntity.src = BackpackCard.getStaticPath(`images/foods/${detail.name}/idle.png`);
    ...
    #startComposeAnimation(frameCount: number) {
        ...
        const animate = (now: number) => {
            ...
            (GEH as any).composeAnimTimer = requestAnimationFrame(animate);
        };
    ...
    #removeCardFromSelection() {
        const pos = GEH.cards.findIndex((card: any) => card === this.card);
        if (pos !== -1) {
    ...
    #addCardToSelection(GameCards: HTMLElement) {
        if (GEH.cards.length >= GEH.maxCardNum) {
            const ToastBox = (window as any).ToastBox;
            if (ToastBox) ToastBox(`最多携带${GEH.maxCardNum}张卡片`);
            return;
        }
    ...
    #performSmoothAnimation(targetRect: DOMRect) {
