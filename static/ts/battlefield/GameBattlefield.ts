...
import { Character, Food, Plate } from "../Foods.js";
...
export default class GameBattlefield extends HTMLElement {
    ...
    ctxFG: CanvasRenderingContext2D | null = null;
    ...
    _lastProgressWidth: string | null = null;
    ...
    #updateWaveNumDisplay(value: number) {
    ...
    showHugeWave = throttle((HugeWaveTag: number) => {
    ...
    showFinalWave = throttle((HugeWaveTag: number) => {
        this.#updateWaveNumDisplay(HugeWaveTag);
    ...
    renderFog(ctx: CanvasRenderingContext2D, fogSrc: string, lightDEG: Int8Array, fogColNum: number, fogBlowInterval: number, fogBlowAnimation: number, fogBlowAnimationLength: number) {
        if (!lightDEG) return;
        let offsetX = 24;
        ...
        for (let i = 0; i < lightDEG.length; i++) {
            ...
            if (x < level.column_num + 1 - fogColNum) continue;
            ...
            if (deg === 1) {
                ...
                if (fog) {
                    ...
                    if (ori) ctx.drawImage(ori, dx, dy, ori.width, ori.height);
                }
            } else if (deg === 0) {
                const fog = GEH.requestDrawImage(fogSrc);
                if (fog) ctx.drawImage(fog, dx, dy, fog.width, fog.height);
            }
        }
    }
    updateBullets = () => {
