...
export default class BulletManager {
     ...
     * 获取指定索引的子弹
     */
    at(index: number): Bullet | undefined {
     ...
     */
    replaceAt(index: number, newBullet: Bullet): void {
        if (index >= 0 && index < this.bullets.length) {
     ...
     */
    updateSimple(
        ctx: IRenderer | CanvasRenderingContext2D,
        maxStackSize: number
    ): void {
        // 重置死亡掩码
        this.deadMask.length = 0;
     ...
     * 获取底层数组的只读视图（用于兼容性）
     */
    getBullets(): ReadonlyArray<Bullet> {
        return this.bullets;
    }
