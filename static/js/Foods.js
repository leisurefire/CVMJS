...
const specialGenerate = function (x, y, star = 0, skillLevel = 0, type = 0) {
    ...
    switch (type) {
        case 0: {
            level.createSpriteAnimation((x * level.row_gap + level.column_start - 20), (y * level.column_gap + level.row_start - 60), "/images/foods/icecream/idle.png", 16, {
                function: () => {
        ...
        case 1: {
            level.createSpriteAnimation((x * level.row_gap + level.column_start - 10), (y * level.column_gap + level.row_start - 40), "/images/foods/groundcoffee/idle.png", 13, {
                function: () => {
                    if (food?.layer_1) {
                        food.layer_1.wakeUp();
                    }
        ...
        case 2: {
            ...
            level.createSpriteAnimation((x * level.row_gap + level.column_start - 2), (y * level.column_gap + level.row_start - 20), "/images/foods/cork/idle.png", 40, {
                function: () => {
...
export class Food {
    ...
    ignored = false;
    ladder;
    static idleLength = 12;
    static artist;
    static storyContributor;
    static type;
    static inside;
    static endLength;
    get inside() {
    ...
    behaviorInterval = null; //行为时间间隔
    remainTime = null; //当前剩余时间
    stateSet = ["idle"]; //状态集合
    ...
    getDamaged(value = 10, origin = null) {
...
export class Character extends Food {
    ...
    CreateOverlayAnim() {
        ...
        if (img) {
            this.ctx.drawImage(img, this.width * this.tick, 0, this.width, this.height, this.x, this.y, this.width, this.height);
        }
    }
    attackCheck() {
        return level.getMiceInRow(this.row).some((mouse) => mouse != null && mouse.length > 0);
    }
    fire() {
...
class BunShooter extends Food {
    ...
    attackCheck() {
        return level.getMiceInRow(this.row).some((mouse) => mouse && mouse.length > 0);
    }
    fire() {
        GEH.requestPlayAudio("zidan");
...
export class Plate extends Food {
    ...
    CreateOverlayAnim() {
        if (this.star >= 4) {
            ...
            if (img) {
                ...
                this.ctx.drawImage(img, img.width / this.starAnimLength * this.starAnimTick, 0, img.width / this.starAnimLength, img.height, x, y, img.width / this.starAnimLength, img.height);
                this.starAnimTick = (this.starAnimTick + 1) % this.starAnimLength;
            }
        }
    }
    behavior() {
        ...
        if (ripple) {
            this.ctx.drawImage(ripple, 72 * this.rippleTick, 0, 72, 39, this.column * level.row_gap + level.column_start - (this.width / 2 - level.row_gap / 2) - 6, this.row * level.column_gap + level.row_start - this.height / 2 + 48, 72, 39);
...
class WineGlass extends Food {
    ...
    behavior() {
        ...
        if (this.state === this.stateSet[3]) {
            ...
            if (sleep) {
                this.ctx.drawImage(sleep, 28 * this.sleepAnimTick, 0, 28, 66, this.x + 18, this.y + 12, 28, 66);
...
class CoffeeCup extends BunShooter {
    ...
    CreateOverlayAnim() {
        ...
        if (this.state === this.stateSet[2]) {
            const sleep = GEH.requestDrawImage(this.sleepAnim);
            if (sleep) {
                this.ctx.drawImage(sleep, 28 * this.sleepAnimTick, 0, 28, 66, this.x + 30, this.y - 30, 28, 66);
                this.sleepAnimTick = (this.sleepAnimTick + 1) % 10;
            }
        }
    }
...
class CoffeePot extends BunShooter {
    ...
    CreateOverlayAnim() {
        ...
        if (this.state === this.stateSet[2]) {
            const sleep = GEH.requestDrawImage(this.sleepAnim);
            if (sleep) {
                this.ctx.drawImage(sleep, 28 * this.sleepAnimTick, 0, 28, 66, this.x + 56, this.y - 24, 28, 66);
                this.sleepAnimTick = (this.sleepAnimTick + 1) % 10;
            }
        }
    }
    ...
    attackCheck() {
        if (level.Mice[this.row] != null) {
            for (let i = this.column; i < Math.min(level.Mice[this.row].length, this.column + 5); i++) {
                if (level.Mice[this.row][i] != null && level.Mice[this.row][i].length > 0) {
                    level?.createSpriteAnimation(this.x + 66, this.y + 10, "/images/bullets/coffeesmog.png", 10, { vertical: true });
                    return true;
                }
            }
        }
...
class GrilledStarfish extends BunShooter {
    ...
    attackCheck() {
        ...
        const { row, column } = this;
        if (Mice[row] && Mice[row].some((mouse) => mouse && mouse.length > 0)) {
            return true;
        }
        if (Mice.some((rowMice) => rowMice && rowMice[column] && rowMice[column].length > 0)) {
            return true;
        }
...
class RotaryCoffeePot extends Food {
    ...
    CreateOverlayAnim() {
        ...
        if (this.state === this.stateSet[2]) {
            const sleep = GEH.requestDrawImage(this.sleepAnim);
            if (sleep) {
                this.ctx.drawImage(sleep, 28 * this.sleepAnimTick, 0, 28, 66, this.x + 88, this.y + 8, 28, 66);
                this.sleepAnimTick = (this.sleepAnimTick + 1) % 10;
            }
        }
    }
...
class SteelWool extends Food {
    ...
    behavior() {
        if (this.target == null) {
            ...
            if (drag) {
                this.ctx.drawImage(drag, 82 * this.rippleTick, 0, 82, 42, level.column_start + this.target.positionX * level.row_gap - 18, level.row_start + this.target.row * level.column_gap + 16, 82, 42);
        ...
        const ripple = GEH.requestDrawImage(this.ripple);
        if (ripple) {
            this.ctx.drawImage(ripple, 72 * this.rippleTick, 0, 72, 39, this.column * level.row_gap + level.column_start - (this.width / 2 - level.row_gap / 2) + 2, this.row * level.column_gap + level.row_start - this.height / 2 + 94, 72, 39);
            this.rippleTick = (this.rippleTick + 1) % 4;
        }
        super.behavior();
    }
...
class TeaCup extends CoffeeCup {
    ...
    behavior() {
        const ripple = GEH.requestDrawImage(this.ripple);
        if (ripple) {
            this.ctx.drawImage(ripple, 72 * this.rippleTick, 0, 72, 39, this.column * level.row_gap + level.column_start - (this.width / 2 - level.row_gap / 2) - 6, this.row * level.column_gap + level.row_start - this.height / 2 + 56, 72, 39);
            this.rippleTick = (this.rippleTick + 1) % 4;
        }
        super.behavior();
    }
...
class Hamburger extends Food {
    ...
    CreateUnderlayAnim(width = 72, height = 36) {
        ...
        if (shadow) {
            ...
            this.ctx.drawImage(shadow, (this.column + 0.5) * level.row_gap + level.column_start - 72 / 2 + left + 4, (this.row + 1) * level.column_gap + level.row_start - 36, 72, 36);
        }
    }
    behavior() {
        if (this.state === this.stateSet[0]) {
    ...
    behavior() {
        if (this.tick === 9) {
            ...
            if (this.column + offset <= level.column_num + 1) {
                level?.createSpriteAnimation(level.column_start + (this.column + offset - 1) * level.row_gap - 2, level.row_start + (this.row) * level.column_gap - 48, "/images/bullets/flame.png", 7);
            }
            if (this.column - offset >= 0) {
                level?.createSpriteAnimation(level.column_start + (this.column - offset) * level.row_gap - 2, level.row_start + (this.row) * level.column_gap - 48, "/images/bullets/flame.png", 7);
            }
...
class ChocolateCannon extends Food {
    ...
    fire() {
        level?.createSpriteAnimation(level.column_start + 8 * level.row_gap - 72, level.row_start + this.row * level.column_gap - 72, "/images/bullets/cannonball.png", 12, {
            func: () => {
                GEH.requestPlayAudio("pijiubao");
...
export const FoodDetails = new Map([
    [0, Stove],
    [1, BunShooter],
    [2, Toast],
    [3, FlourSack],
