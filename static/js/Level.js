...
class Level {
    ...
    constructor(interval, useWebGL = false) {
        if (interval) {
    ...
    async LoadAssets() {
        ...
        const timeoutPromise = i18n.getLoadPromise().then(() => {
            return new Promise((_, reject) => setTimeout(() => reject(new Error(`${i18n.t("GAME_ERROR_CODE_011")}`)), 30000));
        });
        const results = await Promise.race([allSettled, timeoutPromise]);
        let fulfilled = 0;
        let rejected = 0;
        ...
        for (let i = 0; i < wave.mice_type.length; i++) {
            if (wave.mice_num[i] > 0) {
                ...
                for (let j = 0; j < summonNum; j++) {
                    const mouse = getMouseDetails(type).summon();
    ...
    #requestUpdateBattleGround(elapsed) {
        ...
        const renderer = this.Battlefield.renderer || this.Battlefield.ctxBG;
        if (renderer) {
            this._animationManager.updateAnimations(renderer, this.column_num, this.row_num);
        }
        this.#Mice = miceTemp;
    ...
    lightDEGChange(pos, deg) {
        ...
        this.#LightDEG[pos] = (this.#LightDEG[pos] + Math.floor(deg));
    }
    fogBlowAnimation = 0;
