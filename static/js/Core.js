...
export class MaterialButton extends HTMLElement {
    ...
    constructor(text = null, func = null, colored = true, options = null) {
        super();
        this.func = func;
    ...
    `;
        ...
        this.button.addEventListener("mousedown", (ev) => this.#createEffect(ev));
    ...
    `;
        ...
        this.resizeObserver = new ResizeObserver((entries) => {
            for (const entry of entries)
                position(entry.target.offsetWidth);
        });
...
export class MaterialSwitch extends HTMLElement {
    ...
    input;
    threshold = 10;
    constructor(target = null, value = false) {
        super();
        this.target = target;
        const shadow = this.attachShadow({ mode: "open" });
        const box = document.createElement("div");
    ...
    `;
        ...
        this.input.addEventListener("mousemove", (ev) => {
            if (mouseStartX != null) {
                mouseInputX = ev.clientX - mouseStartX;
                if (this.input.checked) {
                    if (mouseInputX < 0)
                        thumb.style.left = `max(0.4rem, calc(1.475rem + ${mouseInputX}px))`;
                }
                else {
                    if (mouseInputX > 0)
                        thumb.style.left = `min(1.475rem, calc(0.4rem + ${mouseInputX}px))`;
                }
            }
        });
        ...
        this.input.addEventListener("mouseleave", () => {
            ...
            if (mouseStartX != null && mouseInputX != null) {
                if (this.input.checked) {
                    if (mouseInputX < 0 && Math.abs(mouseInputX) >= this.threshold)
                        this.input.click();
                }
                else {
                    if (mouseInputX > 0 && Math.abs(mouseInputX) >= this.threshold)
...
export class MaterialDialog extends HTMLElement {
    constructor(options) {
        super();
        const { Title = null, Text, ButtonLabelYes = null, ButtonLabelNo = null, ButtonFuncYes = null, ButtonFuncNo = null, Icon = null, } = options || {};
...
export class MaterialLoader extends HTMLElement {
    constructor() {
        ...
        style.textContent = `
      .material-loader-background { margin:0; position:fixed; height:100%; width:100%; background-image: url("${EventHandler.getStaticPath('images/interface/loader_2.png')}"); background-size:cover; background-repeat:no-repeat; }
    ...
    `;
        ...
        logo.src = EventHandler.getStaticPath('images/logo.svg');
        text.innerText =
...
export class MaterialCard extends HTMLElement {
    ...
    #height = 0;
    constructor(innerHTML = null, customStyle = null) {
        super();
        const shadow = this.attachShadow({ mode: "open" });
        const style = document.createElement("style");
    ...
    `;
        ...
        this.appendChild = (node) => this.#box.appendChild(node);
...
export class MaterialDescription extends HTMLElement {
    ...
    timer = null;
    Style;
    constructor(text, button_text = null) {
        super();
...
export const WarnMessageBox = function (options) {
    const dialog = new MaterialDialog(options);
    document.body.appendChild(dialog);
    return dialog;
};
export const ToastBox = function (text, button_text = null) {
...
class GameReadyPagePrototype extends HTMLElement {
    ...
    constructor() {
      ...
      *{ color: var(--outline); fill: var(--outline); transition: all, .24s cubic-bezier(0.4, 0, 0.2, 1); background-color: var(--background); }
      ...
      .game-ready-page-backpack-cards img { object-fit:cover; border-radius:0.5rem; background-color:#e7bdb1; background-image: url("${EventHandler.getStaticPath('images/undo.svg')}"); background-position:center; background-repeat:no-repeat; }
    ...
    `;
        ...
        const createButtons = () => {
            if (this.#ReselectButton && this.#CancelButton && this.#StartButton)
                return;
            const reselectButton = this.#ReselectButton ??
                new MaterialButton(`${i18n.t("RESELECT")}`, () => {
                    GEH.cards.length = 0;
                    GEH.cardBag.forEach((value) => value.unselect());
    ...
    async initialize(type) {
        ...
        this.#Box.style.backgroundImage =
            type === 99
                ? `linear-gradient(to top, var(--background) 50%, transparent 100%), url(${EventHandler.getStaticPath('images/interface/rouge.png')})`
                : `linear-gradient(to top, var(--background) 50%, transparent 100%), url(${EventHandler.getStaticPath(`images/interface/background_${type || 0}.jpg`)})`;
        // clear lists
        this.GameCards.replaceChildren();
        this.#BackpackCards.scrollTop = 0;
        this.#BackpackCards.replaceChildren();
        GEH.cardBag.forEach((card) => this.#BackpackCards.appendChild(card.entity));
        ...
        const populateLevelInfo = () => {
            ...
            if (this.#Entry.SUGGESTED_TYPE === 0)
                this.#LevelInfoBox.innerHTML += `<a>${EventHandler.icons.get("waterline")}${i18n.t("WATERLINE")}</a>`;
        };
    ...
    #CardSet = () => {
        this.style.display = "none";
        new this.#Entry();
        GEH.cardBag.forEach((c) => c.unselect());
    };
    #onStart() {
        // 等待i18n加载完成后再检查卡片
        const performStartCheck = () => {
            if (!GEH.cards.length)
                return ToastBox(`${i18n.t("GAME_ERROR_CODE_009")}`);
            if (!GEH.debug && this.#Entry.SUGGESTED_TYPE != null) {
                if (this.#Entry.SUGGESTED_TYPE === 0) {
                    const hasPlate = GEH.cards.some((v) => v.name === "plate");
                    if (!hasPlate) {
                        WarnMessageBox({
                    ...
                    const hasMarsh = GEH.cards.some((v) => v.name === "marshmallow");
                    if (!hasMarsh) {
                        WarnMessageBox({
