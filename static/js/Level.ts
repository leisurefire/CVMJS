...
export default class Level {
    ...
    #SpriteAnimationStack: SpriteAnimation[][] = Array.from({ length: (this.row_num * this.column_num + 1) }, () => []);
    constructor() {
        try{
            if(level){
                return level;
    ...
    #updateSpriteAnimations(i:number, j:number, spriteTemp:SpriteAnimation[][]) {
        ...
        for (let i = 0; i < wave.mice_type.length; i++) {
            if (wave.mice_num[i] > 0) {
                ...
                for (let j = 0; j < summonNum; j++) {
                    ...
                    const fullHealth = (getMouseDetails(type).summon()! as Mouse).fullHealth;
                    if (fullHealth > 0) {
    ...
    produceSun(x:number, y:number, num = 25, animation = false) {  //像素坐标x，像素坐标y，是否开启跳一下的动画（仅在卡片生产时开启）
        const sun = new Sun(x, y, num, animation);
        this.#Sun.push(sun);
        return sun;
    }
    requestSummonBullet(type:any, x = 0, y = 0, dam = 20, angle = 0, parameter_1 = null, parameter_2 = null) {
    ...
    #requestRenderFog(ctx: CanvasRenderingContext2D) {
        ...
        for (let i = 0; i < this.#LightDEG.length; i++) {
            ...
            if(this.#LightDEG[i] < 2){
                ...
                if(fog){
                    ctx.drawImage(fog,
                        this.column_start + (x - 1) * (this.column_gap - 4) + 24,
...
export class MapGrid<T extends Food> {
    static typeShield =
...
class SpriteAnimation{
    ...
    isSvg;
    scale;
    constructor(x: number, y: number, src: string, frames: number,
                options :{vertical: undefined|boolean, function: undefined|Function, zIndex:undefined|number, isSvg: boolean|undefined,
...
class GameBattlefield extends HTMLElement{
    ...
    #Parent: any;
    #Cursor:{x:number, y:number, origin:any, picked:boolean} = {
        x: 0,
        y: 0,
        origin: null,
        picked: true,
    }
    private BossBar: HTMLDivElement | undefined;
    private BossProgress: HTMLDivElement | undefined;
    get Cursor(){
        return this.#Cursor;
    }
    ...
    #CountDownElapsed = 0;
    #OverallFront:Mouse|null|undefined = null;
    get OverallFront(){
    ...
    #shovel(ev: MouseEvent|null = null){
        ...
        const Handler = {
            ...
            successFunc: ({positionX, positionY}:{positionX: number, positionY: number})=>{
    ...
    #shortCutHandler = (ev: KeyboardEvent) => {
        if(ev.key === "1"){
            this.#shovel();
        }
    }
    ...
    playPlantAnimation(type = 0, positionX = 0, positionY = 0){
        if(type === 0){
            ...
            level.createSpriteAnimation(x, y,
            "../static/images/interface/smoke", 4, {
                isSvg: true,
                scale: 0.24,
            ...
            const y = (positionY * level.column_gap + level.row_start + 25);
            level.createSpriteAnimation(x, y,
            "../static/images/interface/spray", 4, {
                isSvg: true,
                zIndex : positionY * level.column_num + positionX,
            });
            return true;
    ...
    playCountDownAnimation(elapsed: number){
    ...
    requestCursorTracking(ev:MouseEvent|null, origin:any) {
        if(this.#Cursor.picked){
            return false;
        }
    ...
    #handleCursorClick = (ev: MouseEvent) => {
    ...
    #handleCursorContextMenu = (ev: MouseEvent) => {
    ...
    nextWave = (WaveTag: number, WaveNum: number, IsForwardWaves: boolean, ForwardWaveNum: number) =>{
        if (WaveTag > 1) {
            if(IsForwardWaves){
    ...
    showHugeWave = (HugeWaveTag: number) => {
    ...
    showFinalWave = (HugeWaveTag: number) => {
        this.WaveNum.innerText = HugeWaveTag.toString();
