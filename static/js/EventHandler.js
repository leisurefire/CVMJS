...
class EventHandler {
    ...
    constructor() {
        ...
        window.addEventListener('load', async () => {
            ...
            _a.backgroundMusic = new Audio("../audio/chengzhen.mp3");
    ...
    #loadIcons() {
        this.#iconsLoaded = Promise.all(Array.from(_a.icons.keys()).map(key => fetch(`../images/interface/icons/${key}.svg`)
            .then(res => res.text())
    ...
    requestBackMusicChange(type) {
        ...
        if (music) {
            try {
                _a.backgroundMusic.src = `../audio/${music}.mp3`;
                _a.backgroundMusic.play();
                return true;
            }
            catch (error) {
